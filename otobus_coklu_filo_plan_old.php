	...
	<script type="text/javascript">
		...
		AHReady(function(){
			...
			add_event_on($AH('popup'), '#otobus_sec_action', 'click', function(targ,ev){
				...
				SECILEN_OTOBUSLER = [];
				TABLOLAR = {};
				SENKRONIZASYON_OTOBUS_DATA =  {"dk_oasb":[], "dk_oasa":[], "dk_oasc":[]};
				...
				AHAJAX_V3.req( Base.AJAX_URL + 'coklu_filo_plan.php', manual_serialize({type:'coklu_veri_al', kodlar:JSON.stringify(SECILEN_OTOBUSLER) }), function(res){
					...
					for( var kapi_kod in res.data){
						...
						SENKRONIZASYON_OTOBUS_DATA[Filo_Senkronizasyon.BOLGELER[kapi_kod.substr(0,1)]].push( kapi_kod );
					}
					//console.log(SENKRONIZASYON_OTOBUS_DATA);
					Filo_Senkronizasyon.Manuel_Tetik( SENKRONIZASYON_OTOBUS_DATA, 20, function( SENK_DATA ){
						// console.log( Object.size(TABLOLAR) );
						set_html( $AH('coklu-tables'), "" );
						if( Object.size(TABLOLAR) > 0 ){
							for( var kapi in TABLOLAR ){
								TABLOLAR[kapi].reset();
								var JSON_data = SENK_DATA[Filo_Senkronizasyon.BOLGELER[kapi.substr(0,1)]][kapi];
								if( JSON_data != "BOŞ" ){
									TABLOLAR[kapi].set_JSON_data( SENK_DATA[Filo_Senkronizasyon.BOLGELER[kapi.substr(0,1)]][kapi] ,'no', 'numeric' );
									TABLOLAR[kapi].init();
								}
							} 
						}
